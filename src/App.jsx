...
const COMEBACK_WEEKS = [
  ...
  },
  {
    ...
    days: [
      ...
      { date: '2026-06-11', label: 'Thu 11 Jun', slots: [
        { id: 'w11-thu-b', type: 'breakfast', meal: '', pax: 0, prepBy: '', time: '08:15' },
        { id: 'w11-thu-l', type: 'lunch', meal: '', pax: 0, prepBy: '', time: '12:30' },
        { id: 'w11-thu-s', type: 'pm snacks', meal: '', pax: 0, prepBy: '', time: '14:00' },
      ]},
      { date: '2026-06-12', label: 'Fri 12 Jun', slots: [
        { id: 'w11-fri-b', type: 'breakfast', meal: '', pax: 0, prepBy: '', time: '08:15' },
        { id: 'w11-fri-l', type: 'lunch', meal: '', pax: 0, prepBy: '', time: '12:30' },
        { id: 'w11-fri-s', type: 'pm snacks', meal: '', pax: 0, prepBy: '', time: '14:00' },
      ]},
    ]
  }
]
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px', paddingBottom: 80 }}>
        {[...suppliers].sort((a, b) => {
          ...
          const sa = subView === 'account' ? acctStatus(a) : a.assortmentStatus
          const sb = subView === 'account' ? acctStatus(b) : b.assortmentStatus
    ...
    <div style={{
      ...
      <div onClick={() => setExpanded(e => !e)} style={{ padding: '12px 14px', cursor: 'pointer' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
          ...
          <Pill status={task.status} small onClick={e => { e.stopPropagation(); cycleStatus() }} />
        </div>
      </div>
...
function TasksTab({ tasks, setTasks }) {
  ...
  const priorities = ['all', 'Critical', 'Important']
      ...
      <div style={{ padding: '12px 16px 0', flexShrink: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <div style={{ color: WHITE, fontSize: 13, fontWeight: 700 }}>
            {tasks.filter(t => t.status === 'completed').length}/{tasks.length} completed · {totalPct}%
          </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 12, overflowX: 'auto', paddingBottom: 4 }}>
          {[
            { label: 'owner', val: filterOwner, set: setFilterOwner, opts: owners },
            { label: 'status', val: filterStatus, set: setFilterStatus, opts: statuses },
            { label: 'priority', val: filterPriority, set: setFilterPriority, opts: priorities },
          ].map(f => (
            <select key={f.label} value={f.val} onChange={e => f.set(e.target.value)} style={{
              ...
              {f.opts.map(o => <option key={o} value={o}>{f.label === 'all' ? o : o === 'all' ? `all ${f.label}s` : o}</option>)}
            </select>
          ))}
        </div>
      </div>
          ...
          return (
            <div key={ws} style={{ marginBottom: 20 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                ...
                <span style={{ color: GREY, fontSize: 11 }}>{wsPct}%</span>
              </div>
              <ProgressBar pct={wsPct} height={3} />
              <div style={{ height: 10 }} />
              {wsTasks.map(t => (
...
function ViewToggle({ view, setView }) {
  return (
    <div style={{ display: 'flex', background: '#38383E', borderRadius: 8, padding: 2, gap: 2, flexShrink: 0 }}>
      {['list', 'calendar'].map(v => (
        <button key={v} onClick={() => setView(v)} style={{
          padding: '5px 12px', borderRadius: 6, border: 'none',
          background: view === v ? WHITE : 'transparent',
          color: view === v ? BLACK : GREY,
          fontSize: 11, fontWeight: 700, cursor: 'pointer', textTransform: 'lowercase'
        }}>{v}</button>
      ))}
    </div>
  )
...
function MealsSubTab({ weeks, setWeeks }) {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{
        display: 'flex', alignItems: 'center', padding: '12px 16px', gap: 10, flexShrink: 0,
        borderBottom: `1px solid ${BORDER}`
      }}>
        {view === 'list' && weeks.map((w, i) => (
          <button key={w.weekNum} onClick={() => setActiveWeek(i)} style={{
            background: activeWeek === i ? RED : '#2C2C2E',
            color: WHITE, border: 'none', borderRadius: 20,
            padding: '7px 18px', fontSize: 13, fontWeight: 700, cursor: 'pointer'
          }}>week {w.weekNum}</button>
        ))}
        <div style={{ flex: 1 }} />
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px', paddingBottom: 80 }}>
          {weeks[activeWeek].days.map((day, di) => (
            <div key={day.date} style={{ marginBottom: 20 }}>
              <div style={{ color: WHITE, fontSize: 14, fontWeight: 800, marginBottom: 10, textTransform: 'lowercase' }}>{day.label}</div>
              {day.slots.map((slot, si) => (
                <MealSlot key={slot.id} slot={slot}
                  onUpdate={changes => updateSlot(activeWeek, di, si, changes)} />
              ))}
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{
        ...
        padding: '12px 16px', flexShrink: 0, borderBottom: `1px solid ${BORDER}`
      }}>
        <ViewToggle view={view} setView={setView} />
      </div>
      {view === 'list' ? (
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px', paddingBottom: 80 }}>
          {[
            '📦 order CO2 for delivery on Friday',
            '🥖 order fresh items + bread for Monday of opening week'
          ].map(note => (
            <div key={note} style={{
              background: 'rgba(255,149,0,0.15)', border: `1px solid ${AMBER}`,
              borderRadius: 10, padding: '10px 12px', marginBottom: 10,
              color: '#FFD060', fontSize: 12, fontWeight: 600, lineHeight: 1.4
            }}>{note}</div>
          ))}
          {sortedDates.map(date => (
            <div key={date} style={{ marginBottom: 18 }}>
              <div style={{ color: WHITE, fontSize: 13, fontWeight: 800, marginBottom: 8 }}>{fmtDayLabel(date)}</div>
              <div style={{ background: CARD_BG, borderRadius: 12, border: `1px solid ${BORDER}`, overflow: 'hidden' }}>
                {byDay[date].map((d, i) => (
                  <div key={d.id} style={{
                    display: 'flex', alignItems: 'center', gap: 10, padding: '11px 14px',
                    borderBottom: i < byDay[date].length - 1 ? `1px solid ${BORDER}` : 'none'
                  }}>
                    <div style={{
                      background: '#48484E', borderRadius: 6, padding: '4px 8px',
                      color: WHITE, fontSize: 11, fontWeight: 800, flexShrink: 0
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ color: WHITE, fontSize: 13, fontWeight: 600 }}>{d.supplier}</div>
                      <div style={{ color: GREY, fontSize: 11, marginTop: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.purpose}</div>
                    </div>
                    <Pill status={d.status} small onClick={() => cycleStatus(d.id)} />
                  </div>
                ))}
...
function TrainingCard({ training, onUpdate, onDelete }) {
  ...
  const addPeriod = () => onUpdate({
    periods: [...training.periods, { id: Date.now(), name: '', content: '' }]
  })
  const updatePeriod = (id, changes) => onUpdate({
    periods: training.periods.map(p => p.id === id ? { ...p, ...changes } : p)
  })
  const removePeriod = id => onUpdate({ periods: training.periods.filter(p => p.id !== id) })

  const addLink = () => onUpdate({
    links: [...training.links, { id: Date.now(), label: '', url: '' }]
  })
  const updateLink = (id, changes) => onUpdate({
    links: training.links.map(l => l.id === id ? { ...l, ...changes } : l)
  })
  const removeLink = id => onUpdate({ links: training.links.filter(l => l.id !== id) })

  return (
    <div style={{
      background: CARD_BG, borderRadius: 14, marginBottom: 12,
      border: `1px solid ${BORDER}`, overflow: 'hidden'
    }}>
      <div onClick={() => setExpanded(e => !e)} style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ color: GREY, fontSize: 12, marginTop: 2 }}>
            {training.duration} · {training.periods.length} period{training.periods.length !== 1 ? 's' : ''}
          </div>
        </div>
      ...
      {expanded && (
        <div style={{ padding: '0 16px 16px', borderTop: `1px solid ${BORDER}` }}>
          ...
          <Section label="training name">
            <Input value={training.name} onChange={v => onUpdate({ name: v })} placeholder="Training name" />
          </Section>
          <Section label="total duration">
            <Input value={training.duration} onChange={v => onUpdate({ duration: v })} placeholder="e.g. 2h30" />
          </Section>

          <div style={{ color: GREY, fontSize: 11, fontWeight: 700, letterSpacing: 0.5, marginBottom: 8 }}>PERIODS</div>
          {training.periods.map((p, i) => (
            <div key={p.id} style={{
              background: '#38383E', borderRadius: 10, padding: 12, marginBottom: 8,
              border: `1px solid ${BORDER}`
            }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <span style={{ color: WHITE, fontSize: 12, fontWeight: 600 }}>Period {i + 1}</span>
                <button onClick={() => removePeriod(p.id)} style={{
                  background: 'none', border: 'none', color: RED, fontSize: 16, cursor: 'pointer', padding: 0
                }}>×</button>
              </div>
              <div style={{ marginBottom: 8 }}>
                <Label>name</Label>
                <Input value={p.name} onChange={v => updatePeriod(p.id, { name: v })} placeholder="Period name" />
              </div>
              <div>
                <Label>content</Label>
                <Input multiline value={p.content} onChange={v => updatePeriod(p.id, { content: v })} placeholder="What's covered…" />
              </div>
            </div>
          ))}
          <button onClick={addPeriod} style={{
            background: '#38383E', color: WHITE, border: `1px dashed ${BORDER}`,
            borderRadius: 10, padding: '8px', fontSize: 12, cursor: 'pointer',
            width: '100%', marginBottom: 16
          }}>+ add period</button>

          <div style={{ color: GREY, fontSize: 11, fontWeight: 700, letterSpacing: 0.5, marginBottom: 8 }}>DOCUMENT LINKS</div>
          {training.links.map(l => (
            <div key={l.id} style={{
              ...
              <div style={{ marginBottom: 8 }}>
                <Label>label</Label>
                <Input value={l.label} onChange={v => updateLink(l.id, { label: v })} placeholder="Link label" />
              </div>
              <div>
                <Label>url</Label>
                <Input value={l.url} onChange={v => updateLink(l.id, { url: v })} placeholder="https://…" />
              </div>
              {l.url && <a href={l.url} target="_blank" rel="noreferrer" style={{
                color: RED, fontSize: 12, display: 'block', marginTop: 6,
...
function TrainingTab({ trainings, setTrainings }) {
  const add = () => setTrainings(prev => [...prev, {
    id: `tr${Date.now()}`, name: '', duration: '', periods: [], links: []
  }])
...
export default function App() {
  ...
  const renderTab = (id) => {
    switch (id) {
      case 'cockpit': return <CockpitTab suppliers={suppliers} tasks={tasks} />
      case 'suppliers': return <SuppliersTab suppliers={suppliers} setSuppliers={setSuppliers} />
      case 'tasks': return <TasksTab tasks={tasks} setTasks={setTasks} />
